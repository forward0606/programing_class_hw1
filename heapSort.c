...
void _heapSortInt(int A[], int n){
	...
	for(int i = n-1;i >= 0; i--){
		...
		swapInt(A+t,A+i);
		while(t*2+1 < i){
			if(A[t] < A[t*2+1] || (t *2 +2 < i && A[t] < A[t*2+2])){
				if(t*2 +2 >= i || A[t*2+2] < A[t*2+1]){ 
					swapInt(A+t, A+t*2+1); t = t*2 + 1;
				}
...
void _heapSortStr(char *A[], int n){
	...
	for(int i = n-1;i >= 0; i--){
		...
		swapStr(A+t,A+i);
		while(t*2+1 < i){
			if(strcmp(A[t], A[t*2+1]) < 0 || ( (t*2 +2 < i) && strcmp(A[t], A[t*2+2]) < 0)){
				if(t*2 +2 >= i || strcmp(A[t*2+2], A[t*2+1]) < 0){ 
					swapStr(A+t, A+t*2+1); t = t*2 + 1;
				}
